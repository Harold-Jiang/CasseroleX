...
public record GetHomeMenusQuery : IRequest<HomeMenuDto>
{
...
public class GetHomeMenuTreeHandler : IRequestHandler<GetHomeMenusQuery, HomeMenuDto>
{
    private readonly ICurrentUserService _currentUserService;
    private readonly IApplicationDbContext _context;
    private readonly IMapper _mapper;
    public GetHomeMenuTreeHandler(IApplicationDbContext context,
        IMapper mapper,
    ...
    public async Task<HomeMenuDto> Handle(GetHomeMenusQuery request, CancellationToken cancellationToken)
    {
        // Filter condition 
        Expression<Func<RolePermissions, bool>> where = x => _currentUserService.PermissionIds.Contains(x.Id.ToString()) 
           && x.IsMenu
           && x.Status == Status.normal;
        if (_currentUserService.PermissionIds.Contains("*")) //supadmin
        {
            where = x => x.IsMenu && x.Status == Status.normal;
        }
        ...
        }, request.RefererUrl, request.ShowSubMenu, request.MultipleNav);
    }

    /// <summary>
    /// Get left and top menu bars
    /// </summary>
    private static HomeMenuDto GetSidebarAsync(List<MenuDto> ruleList, Dictionary<string, object> parameters, string? refererUrl, bool showSubMenu, bool multipleNav, string fixedPage = "dashboard")
    {
        HomeMenuDto sidebar = new();
        ...
        if (multipleNav)
        {
            ...
            foreach (var item in topList)
            {
                var childList = GetTreeMenu(
                    ruleList,
                    item.Id,
                    "<li class=\"@class\" pid=\"@pid\"><a @extend href=\"@url@addtabs\" addtabs=\"@id\" class=\"@menuclass\" url=\"@url\" py=\"@py\" pinyin=\"@pinyin\"><i class=\"@icon\"></i> <span>@title <span class=\"pull-right-container\">@caret @badge</span></span></a> @childlist</li>",
    ...
    /// Building Menu Data
    /// </summary>
    public static string GetTreeMenu(List<MenuDto> list, int myid, string itemtpl, List<int>? selectedids = default, List<int>? disabledids = default, string wraptag = "ul", string wrapattr = "", int deeplevel = 0, bool show_submenu = false)
    {
        StringBuilder str = new();
        List<MenuDto> childs = Tree.GetChild(list, myid);
