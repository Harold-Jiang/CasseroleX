...
public class EncryptionService : IEncryptionService
{
    ...
    public virtual bool ValidatePasswordHash(string password, string passwordHash, string saltKey, HashedPasswordFormat passwordFormat = HashedPasswordFormat.SHA1)
    {
        var pwd = CreatePasswordHash(password, saltKey,passwordFormat);
        var isValid = pwd == passwordHash;
        return !isValid ? throw new AccountException("用户名不存在或密码错误") : true;
    }
